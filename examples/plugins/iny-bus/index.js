...
function addEvent(events, ctx) {
    return Object.keys(events)
        ...
        if (typeof event === 'function') {
            return { id: bus.on(name, event.bind(ctx)), name: name };
        }
        if (typeof event.handler !== 'function') {
            return { id: '', name: name };
        }
        if (event.once) {
            return { id: bus.once(name, event.handler.bind(ctx)), name: name };
        }
        return { id: bus.on(name, event.handler.bind(ctx)), name: name };
    })
